  ...
  let make = () =>
    <article>
      ...
      <p>
        ...
        {j` functions when compiled with `->React.string}
        <a href="https://bucklescript.github.io/"> {"BuckleScript"->React.string} </a>
        {j`. It's powered by `->React.string}
      ...
      <p>
        {j`Understanding performance isn't easy, and is made more complicated in
           ...
           appear the same, but may have different underlying implementations.`->React.string}
      </p>
      <p>
        {j`To add your own benchmark tests, you will need to `->React.string}
        <a href="https://github.com/johnridesabike/benchmark-bs">
          {"fork and clone this project's git repository"->React.string}
        </a>
        {j`. Follow the instructions in its README.`->React.string}
      </p>
    </article>
}
