...
function push(vec, x) {
  ...
  var size = vec.size;
  if (tail.length < 32) {
    var tail$1 = JsArray.cloneAndAdd(tail, x);
    var last = tail$1[tail$1.length - 1 | 0];
    return {
            size: size + 1 | 0,
            shift: vec.shift,
            root: vec.root,
            tail: tail$1,
            init: Belt_List.add(vec.init, vec),
            last: Caml_option.some(last)
          };
  }
...
function pop(vec) {
  ...
  if (tail.length > 1) {
    var tail$1 = JsArray.slice(tail, 0, tail.length - 1 | 0);
    var last = tail$1[tail$1.length - 1 | 0];
    return {
            size: size - 1 | 0,
            shift: vec.shift,
            root: vec.root,
            tail: tail$1,
            init: List.tl(vec.init),
            last: Caml_option.some(last)
          };
  }
  var tail$2 = getLeafUnsafe(vec, size - 2 | 0);
  var last$1 = tail$2[tail$2.length - 1 | 0];
  var nr = popTail(size, shift, vec.root);
  var root = nr !== undefined ? nr : ({
        TAG: /* Node */0,
        _0: []
      });
  if (root.TAG !== /* Node */0) {
...
function fromArray(ar) {
  ...
  var last = tail.length >= 1 ? Caml_option.some(tail[tail.length - 1 | 0]) : undefined;
  var i = 0;
  var init_root = {
    TAG: /* Node */0,
    _0: []
